...
pushd "%SCRIPT_DIR%"
dotnet tool install dotnet-charon --local --create-manifest-if-needed >nul
popd

if "%ERRORLEVEL%" NEQ "0" goto Exit_Failure_Dotnet_Restore_Failed


pushd "%SCRIPT_DIR%"
dotnet tool update dotnet-charon --local >nul
popd
