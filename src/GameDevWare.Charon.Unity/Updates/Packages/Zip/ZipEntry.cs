...
namespace GameDevWare.Charon.Unity.Updates.Packages.Zip
{
	...
	internal class ZipEntry
	{
		...
		public bool IsDOSEntry
		{
			get
			{
				return ((this.HostSystem == (int)HostSystemID.Msdos) ||
		...
		private bool HasDosAttributes(int attributes)
		{
			...
			if ((this.known & Known.ExternalAttributes) != 0)
			{
				result |= (((this.HostSystem == (int)HostSystemID.Msdos) ||
					(this.HostSystem == (int)HostSystemID.WindowsNT)) &&
		...
		public int Version
		{
			get
			{
				...
				{
					return this.versionToExtract & 0x00ff;               // Only lower order byte. High order is O/S file system.
				}
				else
				{
					int result = 10;
					...
					else if (this.IsDirectory == true)
					{
						result = 20;
					}
					else if (this.IsCrypted == true)
					{
						result = 20;
					}
		...
		public bool CanDecompress
		{
			get
			{
				return (this.Version <= ZipConstants.VersionMadeBy) &&
					((this.Version == 10) ||
					(this.Version == 11) ||
					(this.Version == 20) ||
					(this.Version == 45) ||
					(this.Version == 51)) &&
					this.IsCompressionMethodSupported();
			}
		}
		...
		public bool IsDirectory
		{
			get
			{
				...
				bool result =
					((nameLength > 0) &&
					((this.name[nameLength - 1] == '/') || (this.name[nameLength - 1] == '\\'))) ||
					this.HasDosAttributes(16)
					;
				return result;
			}
		}
