...
namespace GameDevWare.Charon.Unity.Updates.Packages.Zip.Checksum
{
	...
	internal sealed class Crc32 : IChecksum
	{
		...
		public long Value
		{
			get
			{
				return (long)(this.checkValue ^ crcXor);
			}
		}
		...
		{
			this.checkValue = unchecked(crcTable[(this.checkValue ^ bval) & 0xFF] ^ (this.checkValue >> 8));
		}
		...
		public void Update(ArraySegment<byte> segment)
		{
			...
			while (--count >= 0)
				this.Update(segment.Array[offset++]);
		}
	}
}
