...
namespace GameDevWare.Dynamic.Expressions
{
	public class ExpressionBuilder
	{
		...
		public ExpressionBuilder(IList<ParameterExpression> parameters, Type resultType, Type contextType = null, Type[] knownTypes = null)
		{
			...
			var exposedTypes = new Type[parameters.Count + knownTypes.Length];
			for (var i = 0; i < parameters.Count; i++)
				exposedTypes[i] = parameters[i].Type;
			knownTypes.CopyTo(exposedTypes, parameters.Count);
				...
				foreach (var methodParameter in methodParameters)
				{
					...
					if (node.TryGetValue(methodParameter.Name, out argument))
					{
						...
						if (argument != null && methodParameter.ParameterType == typeof(Type) && TryGetTypeName(argument, out typeName))
							argument = ResolveType(typeName, node);
		...
		private Expression BuildConstant(ExpressionTree node)
		{
			...
			var type = ResolveType(Convert.ToString(typeObj, Format), node);
		...
		private Expression BuildDefault(ExpressionTree node)
		{
			...
			var type = this.ResolveType((string)typeObj, node);
		...
		private Type ResolveType(string typeName, ILineInfo lineInfo)
		{
			...
			if (this.knownTypes.TryGetValue(typeName, out types) == false)
			{
				if (typeName.IndexOf('.') >= 0 || this.TryResolveType("System." + typeName, out type) == false)
					throw new ExpressionParserException(string.Format(Properties.Resources.EXCEPTION_BUILD_UNABLETORESOLVETYPE, typeName, string.Join(", ", this.knownTypes.Keys.ToArray())), lineInfo);
			}
			else if (types.Length > 1)
				throw new ExpressionParserException(string.Format(Properties.Resources.EXCEPTION_BUILD_UNABLETORESOLVETYPEMULTIPLE, typeName, string.Join(", ", Array.ConvertAll(types, t => t.FullName))), lineInfo);
			else
				type = types[0];
